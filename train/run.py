...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        bert_inputs_lens = []
        for text in texts:
            bert_input = text_to_bert_inputs(text, FLAGS.max_seq_length, tokenizer)
            bert_inputs_lens.append(len(preprocess(text, tokenizer)[0]))
        ...
        for text, (input_ids, _, _, _), bert_inputs_len, result in zip(texts, bert_inputs, bert_inputs_lens, results):
            input_ids = input_ids[1:1+bert_inputs_len]
